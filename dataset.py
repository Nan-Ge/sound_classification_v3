...
class KnockDataset_pair(Dataset):
    def __init__(self, root_dir, support_label_set):
        ...
        self.total_npy_files = os.listdir(exp_data_dir)
        ...
        for npy_file in self.total_npy_files:
            ...
            ### 读取exp data
            exp_x_data = np.load(os.path.join(exp_data_dir, npy_file))
            random_index = np.random.randint(0, exp_x_data.shape[0])
            self.total_exp_x_data.append(exp_x_data[random_index])

            ### 读取sim data
            sim_x_data = np.load(os.path.join(sim_data_dir, npy_file))
            self.total_sim_x_data.append(sim_x_data[0])
