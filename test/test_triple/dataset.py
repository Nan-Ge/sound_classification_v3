...
def src_tgt_intersection(dataset_dir, dom):
    ...
    common_files = [file_name for file_name in src_files if file_name in tgt_files]

    # 过滤common数据
    common_files = [file_name for file_name in common_files if len(str(file_name).split('-')) > 3]
...
def load_data(dataset_dir, dom, train_flag):
    ...
    if train_flag == 1:
        ...
        label = list(set(src_y_list))
        for i in range(label.__len__()):
            src_y_total[src_y_total == label[i]] = i
            tgt_y_total[tgt_y_total == label[i]] = i
...
class TripletKnock(Dataset):
    ...
    def __init__(self, knock_dataset, train):
        self.knock_dataset = knock_dataset
        self.train = train
        self.transform = self.knock_dataset.transform

        if self.train:
            self.train_labels = self.knock_dataset.train_labels
            self.train_data = self.knock_dataset.train_data
            self.labels_set = set(self.train_labels.numpy())
            self.label_to_indices = {label: np.where(self.train_labels.numpy() == label)[0]
                                     for label in self.labels_set}

        else:
            self.val_labels = self.knock_dataset.val_labels
            self.val_data = self.knock_dataset.val_data
