...
class FunctionPairSelector(PairSelector):
    def __init__(self, margin):
        super(FunctionPairSelector, self).__init__()
        self.margin = margin

    def get_pairs(self, embeddings, labels):
        exp_embedding = embeddings[0]
        sim_embedding = embeddings[1]

        pair_dist = (exp_embedding - sim_embedding).pow(2).sum(1)

        negative_indices = np.where(pair_dist.cpu() > self.margin)[0]

        if len(negative_indices) == 0:
            negative_indices = np.where(pair_dist.cpu() > 0)[0]
