...
class X_vector(nn.Module):
    def __init__(self, input_dim=40, tdnn_embedding_size=10, n_class=2):
        super(X_vector, self).__init__()
        self.tdnn1 = TDNN(input_dim=input_dim, output_dim=128, context_size=5, dilation=1, dropout_p=0.3)
        self.tdnn2 = TDNN(input_dim=128, output_dim=128, context_size=5, dilation=2, dropout_p=0.3)
        self.tdnn3 = TDNN(input_dim=128, output_dim=128, context_size=7, dilation=3, dropout_p=0.3)
        self.tdnn4 = TDNN(input_dim=128, output_dim=128, context_size=1, dilation=1, dropout_p=0.3)
        self.tdnn5 = TDNN(input_dim=128, output_dim=128, context_size=1, dilation=1, dropout_p=0.3)
        self.segment6 = nn.Linear(256, tdnn_embedding_size)
    ...
    def forward(self, inputs):
        ...
        return x_vec, x_vec, predictions


class DANN_triplet_Net(nn.Module):
    def __init__(self, input_dim, tdnn_embedding_size, triplet_output_size, pair_output_size):
        super(DANN_triplet_Net, self).__init__()
        ### Feature Extractor: TDNN
        ...
        self.label_predictor.add_module('lp_fc1', nn.Linear(tdnn_embedding_size, 128))
        self.label_predictor.add_module('lp_bn1', nn.BatchNorm1d(128))
        ...
        # Output Layer
        self.label_predictor.add_module('lp_fc3', nn.Linear(128, triplet_output_size))
        # self.class_classifier.add_module('c_softmax', nn.LogSoftmax(dim=1))

        ### Domain classifier
        # Layer 1
        self.domain_classifier = nn.Sequential()
        self.domain_classifier.add_module('dc_fc1', nn.Linear(tdnn_embedding_size, 128))
        self.domain_classifier.add_module('dc_bn1', nn.BatchNorm1d(128))
        self.domain_classifier.add_module('dc_prelu1', nn.PReLU())

        # Layer 2
        self.domain_classifier.add_module('dc_fc2', nn.Linear(128, pair_output_size))
